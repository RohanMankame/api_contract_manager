...
export default function ProductDetailsPage() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                // Fetch product contracts
                try {
                    const contractsRes = await productService.getProductContracts(id);
                    setContracts(contractsRes.data?.data?.contracts || []);
                } catch (err) {
                    console.error("Failed to fetch product contracts", err);
                }
    ...
    return (
        <div className="mt-4 text-left space-y-6 pb-12">
            ...
            {/* Product Contracts Grid */}
            <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                    <h3 className="text-lg font-medium text-gray-900">Related Contracts</h3>
                    <p className="text-sm text-gray-500">{contracts.length} contract(s)</p>
                </div>
                ...
                {contracts.length > 0 ? (
                    ...
                    <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50/50 p-12 text-center flex flex-col items-center">
                        <ContractsIcon className="mx-auto h-12 w-12 text-gray-400 " />
                        <h4 className="mt-2 text-sm font-medium text-gray-900">No contracts found</h4>
                        <p className="mt-1 text-sm text-gray-500">This product isn't currently used in any contracts.</p>
                    </div>
                )}
            </div>
